...
contract("TTTController", (accounts) => {
  ...
  describe("play", () => {
    ...
    it("should only allow player who has control to play", async () => {
      const tx = await controller.play(encodePos(0, 0), { from: player1 })
    ...
    it("should not allow player who doesn't have control to play", async () => {
      await expectThrow(controller.play(encodePos(0, 0), { from: player2 }))
    })
    it("control should alternate", async () => {
      await controller.play(encodePos(0, 0), { from: player1 })
      assert.equal(await controller.control(), 2)
      await expectThrow(controller.play(encodePos(0, 1), { from: player1 }))
    })
  })
  describe("payout", () => {
    beforeEach("setup game", async () => {
    ...
    it("should pay the winner when player 1 wins", async () => {
      const bet = await controller.BET_AMOUNT()
      await controller.play(encodePos(0, 0), { from: player1 })
      await controller.play(encodePos(0, 1), { from: player2 })
      await controller.play(encodePos(1, 1), { from: player1 })
      await controller.play(encodePos(0, 2), { from: player2 })
      await controller.play(encodePos(2, 2), { from: player1 })
    ...
    it("should split the payout if match is a draw", async () => {
      const bet = await controller.BET_AMOUNT()
      await controller.play(encodePos(1, 1), { from: player1 })
      await controller.play(encodePos(1, 0), { from: player2 })
      await controller.play(encodePos(0, 0), { from: player1 })
      await controller.play(encodePos(2, 2), { from: player2 })
      await controller.play(encodePos(2, 0), { from: player1 })
      await controller.play(encodePos(0, 2), { from: player2 })
      await controller.play(encodePos(1, 2), { from: player1 })
      await controller.play(encodePos(0, 1), { from: player2 })
      await controller.play(encodePos(2, 1), { from: player1 })
      const tx = await controller.payout({ from: player1 })
      assert.equal(tx.logs[0].event, "LogPayout")
      assert.equal(tx.logs[0].args.player, player1)
